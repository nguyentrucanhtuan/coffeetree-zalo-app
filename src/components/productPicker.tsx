...
export default function productPicker(props: any) {
    ...
    function handleAddToCart() {
        const product = { ...productAddCart, addTopping: toppingList, quantity: quantity };
    ...
    return (
        <>
            <Box sx={{ padding: "10px" }}>
                    ...
                    <Box>
                        ...
                        >
                            {listSize.map((size: any) => (
                                <FormControlLabel key={size.product_id} value={size.product_id} control={<Radio />} label={size.name} />
                            ))}
                        </RadioGroup>
                    </Box>
                }
                ...
                <Box sx={{ display: "flex", width: "100%", marginBottom: "10px", justifyContent: "center", alignItems: "center" }}>
                    <Button variant="text" size="small" onClick={() => handleChangeQuantity(quantity == 1 ? 1 : quantity - 1)}>
                        <RemoveCircleOutlineIcon />
                    </Button>
                    <Box sx={{ marginLeft: "5px", marginRight: "5px" }}>
                        <Typography variant='h6'>{quantity}</Typography>
                    </Box>
                    <Button variant="text" size="small" onClick={() => handleChangeQuantity(quantity + 1)}>
