  ...
  return (
    <Box>
      <Box sx={{ marginBottom: "55px" }}>
        ...
        <List sx={{ width: '100%', bgcolor: 'background.paper', height: screen.height - 200 }} >
          <RadioGroup
          ...
          >
            {addressList.length > 0 && addressList.map((address: any, index) => (
              <Box key={index} sx={{ display: "flex", justifyContent: "space-between", paddingTop: "10px", paddingBottom: "10px" }}>
                <Box sx={{ display: "flex" }}>
                  ...
                  <Box>
                    <Typography>{address.fullname} - {address.phone}</Typography>
                    <Typography variant="subtitle2" sx={{ color: "#00000099" }}>{address.address}</Typography>
                  </Box>
                </Box>
                <Box sx={{ paddingRight: "10px" }} onClick={() => { handleOpenEditAddress(index) }} >
                  <ArrowForwardIosIcon />
                </Box>
              </Box>
            ))}
          ...
          {addressList.length < 3 &&
            <Box sx={{ display: "flex", justifyContent: "space-between", paddingTop: "10px", paddingBottom: "10px" }} onClick={() => { handleAddAddress() }}>
      ...
      <Paper sx={{ position: 'fixed', bottom: 0, left: 0, right: 0 }} elevation={3}>
        <Box sx={{ display: "flex" }}>
          ...
          <Button onClick={() => { handleClose() }} variant="outlined" size="large" sx={{ margin: "10px", width: "100%" }}>
            Đóng
          </Button>
          <Button onClick={() => { handleConfirm() }} variant="contained" size="large" color="success" sx={{ margin: "10px", width: "100%" }}>
            Xác nhận
          </Button>
        </Box>
      </Paper>
      ...
      >
        <AddressPicker addressEditId={addressEditId} toggelDrawerAddress={toggelDrawerAddress}  />
      </Drawer>
    </Box>
  );
}